  ...
  if (snap.empty){
    ...
    for (const t of titles){
      await addDoc(collection(db,'rows'), {
        proj:'Манжерок', order:i, 0:String(i+1), 1:t, 2:'', 3:/новый лист/i.test(t),
        4:'⏳ Не начато', 5:'', 6:'', 7:'', 8:'', seeded:true
      });
      i++;
    }
  }
...
async function subscribeData(){
  ...
  unsubRows = onSnapshot(qRows, snap => { rows = snap.docs.map(d => ({ id:d.id, ...d.data() })); renderKPIs(); renderTable(); });
